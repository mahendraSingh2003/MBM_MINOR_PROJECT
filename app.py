...
def add():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.commit()
            fileNm = id + '.jpg'
            msg = f'''Hi {name},Welcome to the organization.you have been successfully registered in employee database.Thank You.Face Recognition Based Employee Attendance Logger'''
            # send_mail(mail,msg)
            try:
...
YAW_THRESHOLD = 15  # stricter threshold for "straight face"

def gen_frames_takePhoto(user_id="unknown"):
    num = -1
    stable_frames = 0
    required_stable = 5  # number of frames to consider face as stable
    face_last_center = None
    flag = False
    ...
    while True:
        ...
        for face in faces:
            ...
            # --- Stability & center check ---
            face_center = ((face.left() + face.right()) // 2, (face.top() + face.bottom()) // 2)
            frame_center = (size[1] // 2, size[0] // 2)
            dx = abs(face_center[0] - frame_center[0])
            dy = abs(face_center[1] - frame_center[1])

            if abs(yaw) < YAW_THRESHOLD and dx < 120 and dy < 120:  # face straight & near center
                if face_last_center:
                    movement = np.linalg.norm(np.array(face_center) - np.array(face_last_center))
                    if movement < 10:  # face not moving much
                        stable_frames += 1
                    else:
                        stable_frames = 0
                face_last_center = face_center
            else:
                stable_frames = 0
                num = -1  # reset countdown

            # --- Start countdown only after stable ---
            if stable_frames >= required_stable:
                ...
                if num == 0:
                    ...
                    os.makedirs("static/TrainingImages", exist_ok=True)
                    img_path = f"static/TrainingImages/{user_id}.jpg"
                    cv2.imwrite(img_path, pic)

                    encodings = face_recognition.face_encodings(pic)
                    if len(encodings) > 0:
                        os.makedirs("encodings", exist_ok=True)
                        enc_path = f"encodings/{user_id}.npy"
                ...
                cv2.putText(frame, str(num), (150, 200),
...
def takePhoto():
    ...
    return Response(gen_frames_takePhoto(user_id),
...
def gen_frames():
    ...
    def is_head_moved(landmarks, img_shape):
        ...
        return abs(yaw) > 15 or abs(pitch) > 10
    ...
    while True:
        ...
        for encodeFace, faceLoc in zip(encodesCurFrame, facesCurFrame):
            ...
            if matches[matchIndex] and faceDis[matchIndex] < 0.45:
                ...
                if blinked and moved:
                    msg = markEntry(Id)
                    stop_stream = True
                else:
                    msg = "Please blink & turn head!"

                cv2.putText(img, Id, (x1, y2 + 25), cv2.FONT_HERSHEY_TRIPLEX,
                            0.8, (0, 255, 0), 2)
                cv2.putText(img, emp.name, (x1, y2 + 50), cv2.FONT_HERSHEY_TRIPLEX,
                            0.8, (0, 255, 0), 2)
                cv2.rectangle(img, (x1, y1), (x2, y2-4), (0, 255, 0), 2)

                cv2.putText(img, msg, (50, 70),
                            cv2.FONT_HERSHEY_DUPLEX, 1.0, (0, 255, 0), 2)

            else:
                cv2.putText(img, 'Unknown', (x1, y2 + 25),
        ...
        if stop_stream:   # close camera after marking
            cap.release()
            break       
